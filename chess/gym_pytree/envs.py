...
class Chess(gym.Env):
    ...
    def __init__(self, board: chess.Board = chess.Board()) -> None:
    ...
    def tree_flatten(self):
        children = (self._board,)
        aux_data = {'meta': self.meta, 'action_space': self.action_space, 'observation_space': self.observation_space, '_ready': self._ready}
        return (children, aux_data)

    @classmethod
    def tree_unflatten(cls, aux_data, children):
        cls(board=children[0])
        cls.meta = aux_data['meta']
        cls.action_space = aux_data['action_space']
        cls.observation_space = aux_data['observation_space']
        cls._ready = aux_data['_ready']
        return cls


jax.tree_util.register_pytree_node(Chess, Chess.tree_flatten, Chess.tree_unflatten)
